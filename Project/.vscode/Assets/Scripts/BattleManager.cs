...
public class BattleManager : MonoSingleton<BattleManager>
{
    ...
    {   
        MusicManager.Instance.Playsfx(MusicManager.Instance.draw);
        if(num == 0)
        {       
    ...
    {   
        MusicManager.Instance.Playsfx(MusicManager.Instance.put);
    ...
    {   
        MusicManager.Instance.Playsfx(MusicManager.Instance.attack);
        ...
        {   
            ...
            _target.GetComponent<Chara>().attackedIcon.SetActive(true);
            _target.GetComponent<Chara>().attackedIcon.GetComponent<Image>().DOColor(Color.white,0);
            _target.GetComponent<Chara>().attackedIcon.transform.DOScale(new Vector3(0.5f,0.5f,0.5f),1);
            _target.GetComponent<Chara>().attackedIcon.transform.DOScale(new Vector3(1,1,1),1);
            _target.GetComponent<Chara>().attackedIcon.GetComponent<Image>().DOColor(Color.clear,1);
    ...
    public void ExitGame()
   {
    ...
    //AI打牌主函数
    IEnumerator MainMethod() 
        {   
            TurnButton.SetActive(false);
            yield return new WaitForSeconds(2);
            yield return StartCoroutine(nameof(chupai));
            yield return StartCoroutine(nameof(xingdong));
            yield return StartCoroutine(nameof(jieshu));
        }
    //AI出牌
    IEnumerator chupai()
        {   
    ...
    IEnumerator xingdong()
        {
            ...
            foreach (var block in enemyBlocks)
            {
                ...
                if(block.GetComponent<Block>().card != null && block.GetComponent<Block>().card.GetComponent<BattleCard>().attacktimes != 0)
                {
                    ...
                    if(blockIsEmpty == true)  //如果对方没怪
                    {
                        Attack(playerIcon,block.GetComponent<Block>().card);            
                    }
                }
